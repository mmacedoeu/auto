...
def generate_setup_conf(server_history, server_live, server_tag):
    name = server_history[0]["streamName"].split("_")[0]
    cams = merge(server_history, server_live)
    history_list = [c["streamName"] for c in server_history]
    live_list = [c["streamName"] for c in server_live]
    data = { "tag": server_tag, "name": name, "history": server_history, "live": server_live, "cams": cams, "history_list": history_list, "live_list": live_list}
